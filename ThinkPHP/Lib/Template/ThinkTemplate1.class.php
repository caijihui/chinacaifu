...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent) {
        ...
        $tmplContent =  preg_replace_callback('/<!--###literal(\d+)###-->/i',
            function ($r){
    ...
    public function parse($content) {
        ...
        $content    =   preg_replace_callback('/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/i',
            function ($r){
        ...
        $content = preg_replace_callback('/('.$this->config['tmpl_begin'].')([^\d\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/i',
             function ($r) {
    ...
    protected function parseInclude($content) {
        ...
        // 读取模板中的include标签
        $find       =   preg_match_all('/'.$this->config['taglib_begin'].'include\s(.+?)\s*?\/'.$this->config['taglib_end'].'/i',$content,$matches);
        if($find) {
            for($i=0;$i<$find;$i++) {
                $include    =   $matches[1][$i];
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
            ...
            preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/i',
            function ($r) {
                return $this->parseBlock($r[1], $r[2]);
            }
            ,$content);
            // 读取继承模板
            $array      =   $this->parseXmlAttrs($matches[1]);
            ...
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/i',
                function ($r){return $this->replaceBlock($r[1],$r[2]);}
                ,$content);
        }else{
            #$content    =   preg_replace('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/eis',"stripslashes('\\2')",$content);
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/i',
            function ($r){return stripslashes($r[2]);}
                ,$content);
        }
        return $content;
    }
    ...
    public function parseTagLib($tagLib,&$content,$hide=false) {
        ...
        foreach ($tLib->getTags() as $name=>$val){
            ...
            foreach ($tags as $tag){
                ...
                if (!$closeTag){
                    $patterns       = '/'.$begin.$parseTag.$n1.'\/(\s*?)'.$end.'/i';
                    ...
                    $content        = preg_replace_callback($patterns,
                        function ($r) use ($tagLib,$tag){
                            return $this->parseXmlTag($tagLib,$tag,$r[1],'');
                        }
                        ,$content);
                }else{
                    $patterns       = '/'.$begin.$parseTag.$n1.$end.'(.*?)'.$begin.'\/'.$parseTag.'(\s*?)'.$end.'/i';
